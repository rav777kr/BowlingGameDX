...
type
  ...
  private
    ...
    fOwnTable: Boolean;
    function GetTables: IList<TSQLTable>;
  protected
  ...
  public
    ...
    property Tables: IList<TSQLTable> read GetTables;
  end;
...
begin
  ...
  fTables := TCollections.CreateDictionary<string, TSQLTable>([doOwnsValues], 0, TStringComparer.OrdinalIgnoreCase);
  fForeignColumn := nil;
end;
...
// If the entityClass is all that is passed in then we will find the first table object with the
// tablename associated with the entity class. If an associate entity class is referenced twice by two separate
// properties though this is not adequate. Therefore the MemberPath will walk the tree based on the properties provided
// and find the right table object. MemberPath should be in the format
// rootMemberName.childMemberName.GrandChildMemberName and so on. . .
//
...
function TSelectCommand.GetTables: IList<TSQLTable>;
begin
  Result := fTables.Values as IList<TSqlTable>;
end;
...
begin
  ...
  begin
    ...
    //Store the table under the path to the table based on the member names chained together
    //  So for instance given a top level Entity with a field called Store the parent will be empty
    //    and so the table will be stored under the member named Store, but now let's say that Subsidiary also
    //    has an associate called Subsidiary, then the table will be stored under Subsidiary.Store

    //    If then a second instance of Store exists in the top level let's stay orderingStore, FulfillingStore then

    //  the Subsidiary table for each will be stored under OrderingStore.Subsidiary and FulfillingStore.Subsidiary.
    //    This way if a criteria is later created for FulfillingStore.Subsidiary.Number it will find the correct table
    //    during the find method above.
