...
type
  ...
  public
    ...
    procedure SetArrayOfParams(const params: IList<IList<TDBParam>>); override;
...
procedure TFireDACStatementAdapter.SetArrayOfParams(const params: IList<IList<TDBParam>>);
var
  param: TDBParam;
  parms : IEnumerable<TDBParam>;
  ParamsIdx: Integer;
  ParamIdx : Integer;
  First : Boolean;
  I : Integer;
...
begin
  ...
  ParamIdx := 0;
  First := True;
  Statement.Params.ArraySize := Params.Count;
  for parms in params do
  begin
    for Param in Parms do
    begin
      {First time through set the Param type on all parameters}
      if First then
        Statement.Params[ParamIdx].DataType := Param.ParamType;
      Statement.Params[ParamIdx].Values[ParamsIdx] := Param.ToVariant;
      Inc(ParamIdx);
    end;
    First := False;
    ParamIdx := 0;
